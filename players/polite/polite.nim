...
const
  ...
  BrightMinChannel = 30
  BrightMaxChannel = 70
  HoleReductionBonus = 6000
  HoleIncreasePenalty = 9500
  RowCompletionBonus = 3000
  BumpinessPenalty = 250
  HeightPenalty = 25
  DropSearchRadius = 16
  DropHoldRadius = 20
  MinDropScore = 9000
  NotchScore = 13000
  TravelTicksPerColumn = 5
  FallTicksPerCell = 9
  NoGapBonus = 4000
  SupportBonus = 1800
  BelowContactBonus = 1300
  TopContactBonus = 1100
  SideContactBonus = 1000
  NewGapPenalty = 14000
  VerticalStackPenalty = 380
  LocalBumpinessPenalty = 220
  LowPlacementBonus = 15
  ReachPenalty = 25
  MaxFlipAttempts = 16
  ChatIntervalTicks = 450
  PoliteChats = [
    "excuse me, coming through",
    "mind if i fit in here?",
    "let me tidy this up",
    "pardon me",
    "there we go, snug!",
    "just filling this gap",
    "after you!",
...
proc rebuildGlobalMap(bot: var Bot) =
  ...
  if bot.mapScale != scale:
    bot.mapScale = scale
    echo "polite global map scale=", scale
  bot.map = SpriteImage(
    width: mapWidth,
    height: mapHeight,
    label: "global",
...
proc receiveUpdates(
  ...
  while drained < MaxDrainMessages:
    # timeout=0 never reads anything in whisky; 1ms actually drains.
...
proc scorePlacement(
  ...
  ## Scores a candidate by contact, tidiness, and holes. When goalX is
  ## set the candidate is a stepping stone: closing distance toward the
  ## goal column outweighs local niceties (but never justifies holes).
  ...
  # Politeness: reward every face of the piece that rests against an
  # existing block (or floor). Snug notch-filling and tucking under
  # overhangs beat exposed placements.
  var
    contactFaces = 0
    exposedFaces = 0
  for cell in cells:
    let
      ...
      aboveFilled = cells.containsCell(Cell(x: cell.x, y: cell.y - 1)) or
        occupied.occupiedAt(width, height, cell.x, cell.y - 1)
      ...
      rightFilled = cell.x >= width - 1 or
        ...
        occupied.occupiedAt(width, height, cell.x + 1, cell.y)
    if belowFilled:
      result += BelowContactBonus
      inc contactFaces
    elif not cells.containsCell(Cell(x: cell.x, y: cell.y + 1)):
      inc exposedFaces
    if aboveFilled and not cells.containsCell(Cell(x: cell.x, y: cell.y - 1)):
      result += TopContactBonus
      inc contactFaces
    if leftFilled and not cells.containsCell(Cell(x: cell.x - 1, y: cell.y)):
      result += SideContactBonus
      inc contactFaces
    if rightFilled and not cells.containsCell(Cell(x: cell.x + 1, y: cell.y)):
      result += SideContactBonus
      inc contactFaces

  # A perfectly nestled piece (three or more touched faces per cell on
  # average) earns an extra flourish.
  if contactFaces >= cells.len * 2:
    result += SupportBonus
  result -= exposedFaces * 300
  ...
  # Stay low: every cell of altitude above the floor is charged, so the
  # bot spreads wide across the ground and lays pieces flat instead of
  # building upward.
  for cell in cells:
    result -= max(0, bottomRow - cell.y) * VerticalStackPenalty
  if goalX >= 0:
    # Journey mode: reward closing horizontal distance to the goal.
    let
      distBefore = abs(active.originX - goalX)
      distAfter = abs(placement.x - goalX)
    result += (distBefore - distAfter) * 800
  else:
    result -= abs(active.originX - placement.x) * ReachPenalty

proc withinReach(active: ActivePiece, x, y: int): bool =
  ## Returns true when the piece can walk to column x before it lands.
  ## Gravity locks a supported piece instantly (no lock delay), so the
  ## horizontal budget is bounded by the remaining fall time.
  let
    fallTicks = max(0, y - active.originY) * FallTicksPerCell
    maxColumns = fallTicks div TravelTicksPerColumn
  abs(x - active.originX) <= maxColumns
  ...
  for rotation in 0 .. 3:
    ...
    for x in startX .. endX:
      ...
      while occupied.canPlace(
        ...
        inc y
      if limitReach and not active.withinReach(x, y):
        continue
...
proc decideMask(bot: var Bot): uint8 =
  ...
  if globalPlan:
    # Polite planning: hunt the whole board for a genuine notch (a
    # placement whose contact score clearly beats flat ground), then
    # either go there directly or journey toward it across pieces.
    let best = bot.choosePlacement(active)
    if best.found and best.score >= NotchScore and
        not active.withinReach(best.x, best.y):
      # The notch is too far for this piece: lay a clean stepping
      # stone that closes distance, and remember nothing — the next
      # spawn near it re-plans from fresh board state.
      ...
      if placement.found:
        bot.mode = BotDrop
        bot.intent = "journey to " & $best.x
      else:
        bot.mode = BotExplore
        bot.explorationDirection =
          if best.x < active.originX: -1 else: 1
        return bot.explorationMask(active)
    elif best.found and best.score >= NotchScore:
      placement = best
      bot.mode = BotDrop
      bot.intent = "notch found"
    else:
      # No real notch anywhere: settle tidily nearby instead of
      # wandering, so we never mimic stacker's threshold dithering.
      placement = bot.choosePlacement(active, limitReach = true)
      if placement.found:
        bot.mode = BotDrop
        bot.intent = "tidy settle"
      else:
        bot.mode = BotExplore
        return bot.explorationMask(active)
  else:
...
when defined(gui):
  ...
  proc initViewerApp(): ViewerApp =
    ...
    result.window = newWindow(
      title = "Infinite Blocks Polite Viewer",
...
proc runBot(
  ...
  echo "polite player websocket: ", playerUrl.redactedUrl()
  echo "polite global websocket: ", mapUrl.redactedUrl()
  flushFile(stdout)
  var
    bot = initBot()
  ...
  while viewer.viewerOpen():
    try:
      ...
      connected = true
      everConnected = true
      playerWs.send(chatBlob("hello everyone!"), BinaryMessage)
      ...
      while viewer.viewerOpen():
        ...
        let
          ...
          globalUpdated = globalWs.receiveUpdates(bot, false, 1)
        ...
        if nextMask != lastMask:
          ...
          lastMask = nextMask
        if bot.frameTick mod ChatIntervalTicks == 0:
          playerWs.send(
            chatBlob(PoliteChats[bot.rng.rand(PoliteChats.high)]),
            BinaryMessage
          )
      ...
      when not defined(gui):
        if everConnected:
          echo "polite exiting: ", e.msg
          return
      echo "polite reconnecting: ", e.msg
      connected = false
      when defined(gui):
